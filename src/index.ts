...
server.tool(
  "audit-program",
  "Run a full security audit on Solana/Anchor program source code. Returns findings sorted by severity with fix suggestions.",
  {
...
server.tool(
  "check-accounts",
  "Check account validation: owner checks, signer verification, authority, token accounts, duplicate mutable accounts.",
  {
...
server.tool(
  "check-cpi",
  "Analyze Cross-Program Invocations for safety: arbitrary CPI, reentrancy, privilege escalation, program ID validation.",
  {
    source_code: z.string().describe("Rust/Anchor source code"),
  },
  async ({ source_code }) => {
...
server.tool(
  "check-pda",
  "Audit PDA (Program Derived Address) usage: seed confusion, bump storage, collisions.",
  {
    source_code: z.string().describe("Rust/Anchor source code"),
  },
  async ({ source_code }) => {
...
server.tool(
  "check-arithmetic",
  "Detect integer overflow/underflow, precision loss, and unsafe type casting.",
  {
    source_code: z.string().describe("Rust/Anchor source code"),
  },
  async ({ source_code }) => {
...
server.tool(
  "risk-score",
  "Calculate a 0-100 risk score for Solana program code. Quick triage without full details.",
  {
    source_code: z.string().describe("Rust/Anchor source code"),
  },
  async ({ source_code }) => {
...
server.tool(
  "list-patterns",
  "List all vulnerability patterns SolanaShield checks for, optionally filtered by severity or category.",
  {
    severity: z.enum(["critical", "high", "medium", "low"]).optional(),
    category: z.string().optional().describe("Filter by category (e.g., 'CPI Safety', 'Token Safety')"),
  },
...
server.tool(
  "explain-finding",
  "Get detailed explanation, exploit scenario, and remediation for a specific vulnerability pattern.",
  {
...
server.tool(
  "compare-versions",
  "Compare two versions of code to see if fixes resolved previous findings or introduced new issues.",
  {
...
server.tool(
  "generate-fix",
  "Generate a code fix suggestion for a specific finding at a given line.",
  {
    source_code: z.string().describe("Full source code"),
    pattern_id: z.string().describe("Pattern ID to fix (e.g., SOL-C-001)"),
    line_number: z.number().describe("Line number where the finding was detected"),
  },
...
server.tool(
  "check-tokens",
  "Analyze SPL Token operations: authority checks, mint validation, freeze authority, token-2022 compatibility.",
  {
    source_code: z.string().describe("Rust/Anchor source code"),
  },
  async ({ source_code }) => {
...
server.tool(
  "audit-json",
  "Run audit and return structured JSON results for programmatic consumption.",
  {
    source_code: z.string().describe("Rust/Anchor source code"),
  },
  async ({ source_code }) => {
